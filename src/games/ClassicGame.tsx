...
const CONFIG = GAME_CONFIGS.classic
...
export function ClassicGame() {
  ...
  const handleEndGame = useCallback(
    async (ws: WinState, totalCoins: number) => {
      playWin()
      if (ws.winner === 'draw') playDraw()
      setGameOver(true)

      if (gameMode === 'vs-ai') {
        const result = ws.winner === 'p1' ? 'win' : ws.winner === 'draw' ? 'draw' : 'loss'
        const outcome = recordResult('classic', result, totalCoins)
        if (outcome) {
          setGameResult({ xpGained: outcome.xpGained, newAchievements: outcome.newAchievements })
        }
      }
    },
  ...
  const handleColumnClick = useCallback(
    (col: number) => {
      ...
      setTimeout(() => {
        ...
        if (ws.winner !== null) {
          setWinState(ws)
          handleEndGame(ws, newCoins)
        } else {
          setCurrentPlayer((p) => (p === 'p1' ? 'p2' : 'p1'))
        }
      }, 420)
    },
    [board, currentPlayer, winState.winner, droppingCell, coinsDropped, playDrop, handleEndGame],
  )
  ...
  }, [board, currentPlayer, gameMode, difficulty, winState.winner, droppingCell, gameOver, coinsDropped, playDrop, handleEndGame])
    ...
    return (
      <PageTransition className="max-w-md mx-auto px-4 py-10 flex flex-col items-center gap-6">
        <div className="text-center">
          ...
          <h1 className="text-3xl font-black text-white">Classic Connect 4</h1>
          <p className="text-gray-400 mt-1">7×6 board · Connect 4 to win</p>
        </div>
        ...
        <div className="w-full flex flex-col gap-3">
          <Button
            ...
            size="xl"
            fullWidth
            onClick={() => { resetGame(); setGameMode('vs-ai') }}
          >
            🤖 vs AI
          </Button>
          <Button
            ...
            size="xl"
            fullWidth
            onClick={() => { resetGame(); setGameMode('two-player') }}
          >
              ...
              <button
                ...
                className={[
                  ...
                  difficulty === d
                    ...
                    : 'border-navy-700 bg-navy-800 text-gray-400 hover:border-navy-500',
                ].join(' ')}
              >
                {d === 'easy' ? '😊' : d === 'medium' ? '🧠' : '🔥'} {d}
              </button>
  ...
  return (
    <PageTransition className="max-w-2xl mx-auto px-4 py-4 flex flex-col items-center gap-4">
      ...
      {isAITurn && !gameOver && (
        <p className="text-sm text-gray-400 animate-pulse font-semibold">
          🤖 AI is thinking…
        </p>
      )}
